...
const whyTall = [
  {
    ...
    color: "#10B981", iconBg: "#ECFDF5", badgeText: "#065F46", badgeBg: "#ECFDF5",
  },
  {
...
function DotGrid({ className = "", color = "#0A4D2E", opacity = 0.05, size = 22 }) {
  return (
    <div
      ...
      style={{
        backgroundImage: `radial-gradient(circle,${color}${Math.round(opacity * 255).toString(16).padStart(2, "0")} 1px,transparent 1px)`,
      ...
      <div className="flex flex-col bg-white/95 backdrop-blur-xl border border-slate-100 shadow-md rounded-2xl p-1.5 gap-0.5">
        {socialLinks.map(({ label, href, icon, hoverColor }) => (
          <a
            ...
            aria-label={`${label} – Gnanodaya School`}
      ...
      <div className="flex items-center gap-1 bg-white/95 backdrop-blur-xl border border-slate-100 shadow-xl rounded-2xl px-2.5 py-2">
        {socialLinks.map(({ label, href, icon }) => (
          <a
            ...
            rel="noopener noreferrer"
            aria-label={label}
            className="w-10 h-10 flex items-center justify-center rounded-xl text-[#0A4D2E] hover:bg-[#0A4D2E] hover:text-white active:scale-90 transition-all"
          >
            {icon}
          </a>
        ))}
  ...
  return (
    <dl className="space-y-2">
      {faqData.map((faq, i) => (
        <div
          key={i}
          className={`rounded-xl border overflow-hidden transition-all duration-300 ${open === i ? 'border-[#0A4D2E] shadow-md' : 'border-slate-100 hover:border-[#0A4D2E]/25'} bg-white`}
        >
          <dt>
            <button
            ...
            >
              <span className={`font-bold text-sm leading-snug transition-colors ${open === i ? 'text-[#0A4D2E]' : 'text-[#1a2332] group-hover:text-[#0A4D2E]'}`}>
                {faq.q}
              </span>
              <motion.span
                animate={{ rotate: open === i ? 45 : 0 }}
                transition={{ duration: 0.2 }}
                className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center font-black text-sm transition-colors ${open === i ? 'bg-[#0A4D2E] text-white' : 'bg-slate-100 text-slate-400'}`}
              >
                +
              </motion.span>
            </button>
...
function SectionHeader({ eyebrow, title, highlight, desc, center = false }) {
  return (
    <div className={`mb-8 ${center ? "text-center" : ""}`}>
      {eyebrow && (
        <div className="inline-flex items-center gap-2 mb-3 px-3.5 py-1.5 rounded-full border border-[#0A4D2E]/12 bg-[#0A4D2E]/5">
      ...
      <h2 className="text-3xl sm:text-5xl font-black text-[#0A4D2E] tracking-tighter uppercase leading-none">
        {title}{" "}
        {highlight && <span className="text-[#FFC107]">{highlight}</span>}
      </h2>
      {desc && (
        <p className="text-[13px] mt-2.5 max-w-xl leading-relaxed" style={{ color: "#5a6472" }}>{desc}</p>
      )}
    </div>
  );
}
...
// ─── MINI CARD ────────────────────────────────────────────────────────────────
// Matches reference image: thick left colored border, icon in rounded bg box,
// bold title in dark color, lighter subtitle.
function MiniCard({ icon, title, sub, color, iconBg, linkTo }) {
  const inner = (
    <div
      className="group flex items-center gap-4 bg-white rounded-2xl px-6 py-5 h-full transition-all duration-200 hover:-translate-y-1 hover:shadow-md"
      style={{
    ...
    >
      {/* Icon box */}
      <div
        className="w-12 h-12 shrink-0 rounded-xl flex items-center justify-center text-3xl group-hover:scale-110 transition-transform duration-200"
      ...
      <div className="min-w-0">
        <p className="font-black text-[15px] leading-tight truncate" style={{ color: "#0A4D2E" }}>{title}</p>
        <p className="text-[12px] leading-tight mt-0.5 truncate" style={{ color: "#6b7280" }}>{sub}</p>
      </div>
    </div>
  );
...
// ─── TALL CARD ────────────────────────────────────────────────────────────────
// Matches reference image: left border 8px, icon row with badge, animated
// underline, bullet list, decorative circle bottom-right.
function TallCard({ icon, title, badge, bullets, color, iconBg, badgeText, badgeBg, linkTo }) {
  const inner = (
    <div
      className="group relative h-full bg-white rounded-2xl overflow-hidden flex flex-col transition-all duration-300"
      style={{
    ...
    >
      <div className="p-5 sm:p-6 flex flex-col gap-4 flex-1">
        ...
        <div className="flex items-start justify-between gap-2">
          <div
            className="w-14 h-14 rounded-2xl flex items-center justify-center text-[1.8rem] shrink-0 group-hover:scale-110 transition-transform duration-200"
            style={{ background: iconBg, border: `1.5px solid ${color}22` }}
          >
            {icon}
          </div>
          <span
            className="text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border mt-1 shrink-0"
            style={{ background: badgeBg, color: badgeText, borderColor: `${color}55` }}
          >
            {badge}
          </span>
        </div>
        ...
        <div>
          <h3
            className="text-[16px] sm:text-[17px] font-black leading-tight mb-2"
            style={{ color: color }}
          >
            {title}
          </h3>
          <div
            className="h-[1.5px] w-10 rounded-full group-hover:w-14 transition-all duration-300"
        ...
        <ul className="space-y-2.5 flex-1">
          {bullets.map((b, j) => (
            <li key={j} className="flex items-center gap-3 text-[13px] font-medium" style={{ color: "#4b5563" }}>
        ...
        <div
          aria-hidden="true"
          className="absolute bottom-4 right-4 w-20 h-20 rounded-full opacity-[0.09] pointer-events-none"
          style={{ background: color }}
        />
      </div>
    </div>
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -6, transition: { type: "spring", stiffness: 340, damping: 18 } }}
      className="h-full"
    >
      {linkTo
        ? <Link to={linkTo} className="block h-full">{inner}</Link>
...
// ─── CARD GRID SECTION ────────────────────────────────────────────────────────
function CardGridSection({ eyebrow, title, highlight, desc, miniCards, tallCards, miniLinkTo }) {
  return (
    <section className="py-8 sm:py-12 px-4 sm:px-6 max-w-7xl mx-auto">
      <motion.div {...fadeUp(0)}>
        <SectionHeader eyebrow={eyebrow} title={title} highlight={highlight} desc={desc} />
      </motion.div>
      ...
      >
        {miniCards.map((item) => (
          ...
          >
            <MiniCard {...item} linkTo={miniLinkTo} />
          </motion.div>
        ))}
      </motion.div>
  ...
  return (
    <>
      ...
      <main className="w-full bg-[#F8FAFC] overflow-x-hidden antialiased" id="main-content" role="main">
        ...
        <a
          href="#main-content"
          className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-[99999] focus:px-4 focus:py-2 focus:bg-[#0A4D2E] focus:text-white focus:rounded-lg"
        >
        ...
        {/* Scroll progress bar */}
        <motion.div
          className="fixed top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-[#0A4D2E] via-[#FFC107] to-[#0A4D2E] origin-left z-[9999]"
          style={{ scaleX }}
          aria-hidden="true"
        />
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-4">
                  <motion.div
                    ...
                    className="h-[3px] bg-[#FFC107] rounded-full"
                  />
                  <span className="text-[#0A4D2E] font-black tracking-[0.35em] text-[9px] sm:text-[10px] uppercase">
                    Gnanodaya Excellence
                  </span>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  <a
                    href="/about"
                    className="group relative px-6 sm:px-8 py-3 bg-[#0A4D2E] text-white rounded-xl font-black uppercase text-[9px] tracking-[0.2em] overflow-hidden shadow-lg hover:shadow-xl transition-shadow"
                  >
                  ...
                  <a
                    href="/contact"
                    className="px-6 sm:px-8 py-3 border-2 border-[#0A4D2E] text-[#0A4D2E] rounded-xl font-black uppercase text-[9px] tracking-[0.2em] hover:bg-[#0A4D2E] hover:text-white transition-all"
                  >
                    Contact Admissions
                  </a>
                </div>
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    <a
                      href="/registration"
                      className="text-[9px] font-bold uppercase text-white/50 group-hover:text-[#FFC107] transition-all"
                    >
                      Secure Your Seat →
                    </a>
                  </div>
        ...
        <section className="py-6 sm:py-10 px-4 sm:px-6 max-w-7xl mx-auto">
          <motion.div
            {...fadeUp(0)}
            className="relative bg-[#0A4D2E] rounded-[1.5rem] sm:rounded-[2rem] px-6 sm:px-12 py-8 sm:py-10 overflow-hidden"
            style={{ boxShadow: '0 12px 48px rgba(10,77,46,0.22)' }}
          >
        ...
        {/* ══════════════════════════════════════════════════════
            4. WHY CHOOSE US — THE GNANODAYA DIFFERENCE
        ══════════════════════════════════════════════════════ */}
        <CardGridSection
          eyebrow="Why Parents Choose Us"
          title="The Gnanodaya"
          highlight="Difference"
          desc="Six compelling reasons why Madanapalle families have trusted us for over 35 years."
          miniCards={whyMini}
          tallCards={whyTall}
          miniLinkTo="/about/principal-message"
        />
        ...
        <section className="py-6 sm:py-10 px-4 sm:px-6 max-w-7xl mx-auto">
          ...
          >
            ...
            <div className="relative h-[240px] sm:h-[320px] lg:h-auto overflow-hidden">
              <motion.img
                src={homeprincipal.src || homeprincipal}
                alt="School Principal – Gnanodaya School"
                width={720}
                height={900}
              ...
              <div className="absolute bottom-5 left-5 sm:bottom-8 sm:left-8">
                ...
                <p className="text-[#FFC107] font-bold uppercase tracking-widest text-[9px] mt-0.5">
            ...
            <div className="relative p-7 sm:p-10 md:p-14 flex flex-col justify-center overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <blockquote
                  className="text-sm sm:text-base italic leading-relaxed mb-5 border-l-4 border-[#FFC107]/60 pl-4"
                  style={{ color: "#5a6472" }}
                >
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {["AP State Board", "Holistic Education", "Character Building"].map(tag => (
                    <span
                      key={tag}
                      className="text-[9px] font-black uppercase tracking-widest bg-[#0A4D2E]/5 text-[#0A4D2E] px-2.5 py-1.5 rounded-full border border-[#0A4D2E]/10"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-2 text-[#0A4D2E] font-black uppercase tracking-[0.25em] text-[9px] border-b-2 border-[#FFC107] pb-0.5 hover:text-[#B8860B] transition-colors"
                >
                  Read Full Vision →
                </Link>
              </div>
        ...
        {/* ══════════════════════════════════════════════════════
            6. FACILITIES — WORLD-CLASS
        ══════════════════════════════════════════════════════ */}
        <CardGridSection
          eyebrow="Our Campus, Madanapalle"
          title="World-Class"
          highlight="Facilities"
          desc="Modern infrastructure designed to inspire curiosity and support all-round student development."
          miniCards={facMini}
          tallCards={facTall}
        />
        ...
        <section className="pb-10 sm:pb-16 px-4 sm:px-6">
          ...
          >
            ...
            <p
              className="relative z-10 text-sm sm:text-base mb-6 max-w-sm mx-auto leading-relaxed"
            ...
            <div className="relative z-10 flex flex-wrap justify-center gap-3">
              <Link
                to="/contact"
                className="group relative px-7 sm:px-10 py-3.5 bg-[#FFC107] text-[#0A4D2E] rounded-full font-black uppercase tracking-[0.2em] text-[10px] overflow-hidden shadow-xl"
              >
              ...
              <a
                href="/registration"
                className="px-7 sm:px-10 py-3.5 border-2 border-white/25 text-white rounded-full font-black uppercase tracking-[0.2em] text-[10px] hover:border-[#FFC107] hover:text-[#FFC107] transition-all"
              >
                Apply Online
              </a>
            </div>
